...
function readTabHTML() {
  ...
  const html2canvas = init();

  const body = document.documentElement.body;
  
  function removeNonAlphanumeric(inputString, keepSpaces = false) {
    const normalRegex = /[^a-zA-Z0-9]/g;
    const withSpacesRegex = /[^a-zA-Z0-9 ]/g;
    const expression = keepSpaces ? withSpacesRegex : normalRegex;
    const result = inputString.replace(expression, '');
    if (keepSpaces) {
      return result.trim();
    }
    return result;
  ...
  const capture = async ($target) => {
    //await import("../public/vendor/html2canvas");
    
    const screenshotTarget = $target || document.body;
    const canvas = await html2canvas(screenshotTarget);
    const base64image = canvas.toDataURL('image/png'); // here is the most important part because if you dont replace you will get a DOM 18 exception.;
    return base64image;
  };
  ...
  const download = (base64) => {
    ...
    var name =
    removeNonAlphanumeric(document.title.replaceAll(' ', '-')) +
    '-screenshot.png';
